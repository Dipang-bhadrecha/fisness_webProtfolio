...
import { Paginated, PlatformUser, listUsers } from "@/lib/admin/api";
...
export default function UsersPage() {
  ...
  const [result, setResult] = useState<Paginated<PlatformUser> | null>(null);
  const [error, setError] = useState<string | null>(null);

  const load = useCallback(() => {
    if (!token) return;
    listUsers(token, { search: search || undefined, page, limit: LIMIT })
      .then(setResult)
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <h1 className="font-display text-2xl font-bold text-ink dark:text-slate-100">Users</h1>
        <div className="relative">
          ...
          <input
            ...
            className="w-64 rounded-xl border border-muted-faint/50 dark:border-slate-700 bg-transparent py-2.5 pl-9 pr-3.5 text-sm text-ink dark:text-slate-100 focus:outline-none focus:border-teal"
          />
        </div>
      </div>
      ...
      {!result ? (
        ...
        <>
          <UsersTable items={result.data} token={token!} onChanged={load} />

          <div className="mt-4 flex items-center justify-between text-sm text-muted dark:text-slate-400">
            <p>{result.total.toLocaleString()} users total</p>
