...
import { Paginated, PlatformBoat, listBoats } from "@/lib/admin/api";
...
export default function BoatsPage() {
  ...
  const [result, setResult] = useState<Paginated<PlatformBoat> | null>(null);
  const [error, setError] = useState<string | null>(null);

  const load = useCallback(() => {
    if (!token) return;
    listBoats(token, { search: search || undefined, page, limit: LIMIT })
      .then(setResult)
      .catch((err) => setError(err instanceof Error ? err.message : "Could not load boats"));
  }, [token, search, page]);
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <h1 className="font-display text-2xl font-bold text-ink dark:text-slate-100">Boats</h1>
        <div className="relative">
          ...
          <input
            ...
            placeholder="Search by boat or owner name…"
